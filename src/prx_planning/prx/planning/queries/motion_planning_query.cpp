...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void motion_planning_query_t::setup(sim::state_t* start_state, sim::state_t* goal_state, query_collision_type_t q_collision_type, query_type_t q_type)
        {

            // PRX_WARN_S("setup motion_planning_query_t");
            PRX_ASSERT(goal != NULL);
            copy_start(start_state);
            goal->copy_goal_state(goal_state);
            this->q_collision_type = q_collision_type;
            this->q_type = q_type;
            clear();
        }

        void motion_planning_query_t::setup(sim::state_t* start_state, sim::state_t* goal_state)
        {
            setup(start_state, goal_state, q_collision_type, q_type);
        }
        ...
        {
            if(this->state_space != NULL && state_space->get_space_name() != this->state_space->get_space_name())
        ...
        void motion_planning_query_t::copy_start(const sim::state_t* start)
        {
