    ...
    namespace plan
    {
        ...
        namespace comm
        {
            ...
            void planning_comm_t::link_application(planning_application_t* parent_app)
            {
                ...
                request_ground_truth_client = node.serviceClient<prx_simulation::request_ground_truth_srv>("/simulation/request_ground_truth");
...
//                PRX_DEBUG_COLOR("Publish graph)  builder_node: " << graph_builder_planning_node << "   time: " << start_send_time, PRX_TEXT_BROWN);
            ...
            std::vector<double> planning_comm_t::request_ground_truth()
            {

                PRX_DEBUG_COLOR("Starting ground truth call", PRX_TEXT_GREEN);
                request_ground_truth_client.waitForExistence(ros::Duration(5));
                request_ground_truth_server.request.consumer_name = planning_app->consumer_path;
                std::vector<double> state_vec;
                if (request_ground_truth_client.call(request_ground_truth_server))
                {
                    
                    state_vec =  planning_app->sim_to_plan_state(request_ground_truth_server.response.full_state.elements);
                    PRX_DEBUG_COLOR("Successful ground truth call "<<state_vec.size()<<" :: "<<request_ground_truth_server.response.full_state.elements.size(), PRX_TEXT_GREEN);
                    for (int i = 0; i< request_ground_truth_server.response.full_state.elements.size(); ++i)
                    {
                        PRX_DEBUG_COLOR(request_ground_truth_server.response.full_state.elements[i] << " ::: "<< state_vec[i], PRX_TEXT_BROWN);
                    }
                }
                else
                {
                    PRX_ERROR_S("Ground truth service call failed.");
                }
