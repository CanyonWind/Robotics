...
PLUGINLIB_EXPORT_CLASS(prx::plan::world_model_t, prx::plan::world_model_t)
...
namespace prx
{
    ...
    namespace plan
    {
        ...
        pluginlib::ClassLoader<world_model_t> world_model_t::loader("prx_planning","prx::plan::world_model_t");
            ...
            if( world_model_reader->has_attribute("planning_contexts") )
            {
                ...
                foreach(const parameter_reader_t::reader_map_t::value_type key_value, contexts_map)
                {
                    ...
                    foreach(system_ptr_t system_pointer, systems_list)
                    {
                        ...
                        PRX_INFO_S("--Examine: "<<system_pointer->get_pathname() );
                    ...
                    //if the simulator has it's own state, that must be included
                    // if(simulator->get_controller_state_space()!=NULL)
                    // {
                    //     space_t* space = simulator->get_controller_state_space();

                    //     planning_spaces.push_back(space);
                    //     state_mappings.push_back(new full_mapping_t());
                    //     state_mappings.back()->domain = (state_mappings.back()->range) = space->get_dimension();
                    //     state_mappings.back()->output_space_name = space->get_space_name();
                    //     state_mappings.back()->preimage_interval = full_state_space->get_subspace(space);
                    //     state_mappings.back()->init_spaces();
                    ...
                    // PRX_INFO_S("STATE: "<<*contexts[key_value.first].planning_state_space);
                    // PRX_INFO_S("CONTROL: "<<*contexts[key_value.first].planning_control_space);
                    // PRX_INFO_S("STATE: "<<*simulator->get_state_space());
                    // PRX_INFO_S("CONTROL: "<<*simulator->get_control_space());
                }
            }

            // TODO: We might want to move detection and response variables to abstract simulator class,
            // then we can just query simulator_t to see if it collision detects rather than casting to default_simulator_t first. AK
            if( dynamic_cast<default_simulator_t*>(simulator) != NULL && dynamic_cast<default_simulator_t*>(simulator)->simulator_collision_detects())
        ...
        void world_model_t::propagate_plan(const state_t * const state, const plan_t& plan, trajectory_t& traj)
        {
            this->push_state(state);
            state_t* initial = contexts[context_name].temp_state;
            ...
            foreach(plan_step_t step, plan)
            {
                ...
                if( steps > 0 )
                {
                    if( first_step )
                    {
                        ...
                        traj.copy_onto_back(initial);
                        if( phys_based_sim )
                            traj.set_collision(traj.in_collision() || !selected_state_space->satisfies_bounds(initial,true) || simulator->in_collision());
                    }
                    if( phys_based_sim || !use_optimized_propagate )
                    {
                        for( ; i < steps; i++ )
...
//            selected_state_space->copy_from_point(goal);
            ...
            // TODO: Why doesn't this work? Embeddings I imagine.
        ...
        void world_model_t::stitch_spaces(const std::vector<util::space_t*>& output_spaces, const std::vector<util::space_point_t*>& output_points,const std::vector<util::space_t*>& input_spaces, const std::vector<util::space_point_t*>& input_points)
        {
            PRX_ASSERT(input_spaces.size() == input_points.size());
            PRX_ASSERT(output_spaces.size() == output_points.size());
            for(unsigned i =0; i < input_spaces.size(); ++i)
            {
                input_spaces[i]->copy_from_point(input_points[i]);
            }

            for(unsigned i =0; i < output_spaces.size(); ++i)
            {
                output_spaces[i]->copy_to_point(output_points[i]);
            }
        }
        void world_model_t::convert_spaces(const util::space_t* output_space, util::space_point_t* output_point, const util::space_t* input_space, const util::space_point_t* input_point)
        {
            input_space->copy_from_point(input_point);
            output_space->copy_to_point(output_point);
        }
        ...
        pluginlib::ClassLoader<world_model_t>& world_model_t::get_loader()
        {
            return loader;
        }
