...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void planning_application_t::init(const parameter_reader_t* reader)
        {
            ...
            model = (world_model_t*)reader->create_from_loader<world_model_t > ("world_model", "prx_planning");
        ...
        std::vector<double> planning_application_t::sim_to_plan_state(const std::vector<double>& state, std::string c_path)
        {
            ...
            model->use_context(this->consumer_to_space_name_mapping[c_path]);
            ...
            model->get_state_space()->copy_to_vector(new_state);
