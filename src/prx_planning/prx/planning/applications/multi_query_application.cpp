...
#include "prx/planning/applications/multi_query_application.hpp"
...
namespace prx
{
    ...
    namespace plan
    {
        ...
        PLUGINLIB_EXPORT_CLASS( prx::plan::multi_query_application_t, prx::plan::planning_application_t)

        using namespace comm;

        multi_query_application_t::multi_query_application_t()
        {
            interruptable = false;
        }
        ...
        void multi_query_application_t::init(const parameter_reader_t* reader)
        {
        ...
        void multi_query_application_t::execute()
        {
            ...
            while(ros::ok())
            {
                dynamic_cast<motion_planning_query_t*>(root_queries[0])->plan.clear();
                this->root_task->resolve_query();
                if ( dynamic_cast< prx::packages::manipulation::simple_pap_tp_t* >(this->root_task) != NULL )
                {
                     dynamic_cast< prx::packages::manipulation::simple_pap_tp_t* >(this->root_task)->set_wait_for_manipulation(true);
                }
