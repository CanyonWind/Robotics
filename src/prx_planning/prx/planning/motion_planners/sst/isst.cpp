...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void isst_t::init(const parameter_reader_t* reader,const parameter_reader_t* template_reader)
        {
            ...
            reset_count = parameters::get_attribute_as<unsigned>("reset_count",reader,template_reader,PRX_INFINITY);
        ...
        {
            tree_vertex_index_t best_goal = ((const tree_node_t*)metric->single_query(input_query->get_goal()->get_first_goal_point()))->get_index();
        ...
        void isst_t::resolve_query()
        {
            ...
            {                
                ...
                // PRX_INFO_S("cost: "<<get_vertex(new_v)->cost<<" h_val_graph: "<<get_vertex(new_v)->h_value<<" h_val_function: "<<metric->distance_function(tree[new_v]->point,input_query->get_goal()->get_first_goal_point()));
            }
                // PRX_INFO_S("----------");
        ...
        {            
    	    ...
    	    if(solution_found)
    		  return;
            //sample a state
            if( !sst_only && !real_solution && roll_weighted_die(boost::assign::list_of(goal_bias_rate)(1 - goal_bias_rate), false) == 0 )
            {
                    ...
                    if(!collision_checking || ( traj_cost<PRX_INFINITY && validity_checker->is_valid(trajectory) && trajectory.size()>1))
                    {
                        valid--;
                            ...
                            if(!real_solution && input_query->get_goal()->satisfied(end_state))
                            {
                                ...
                                PRX_INFO_S("First Solution Found at Time: "<<first_solution_time<<" s Iteration: "<<iteration_count<<" Cost: "<<get_vertex(best_goal)->cost);
                                PRX_INFO_S("Solution End State: "<<state_space->print_point(end_state));
                                purge_nodes();
                                clock.reset();
				// solution_found =true;
                            }
                            else if(real_solution && get_vertex(best_goal)->cost > get_vertex(v)->cost && input_query->get_goal()->satisfied(end_state) )
                            {
                                best_goal = v;
                                time_elapsed+=clock.measure();
                                PRX_INFO_S("Next Solution Found at Time: "<<time_elapsed<<" s Iteration: "<<iteration_count<<" Cost: "<<get_vertex(best_goal)->cost);
                                purge_nodes();
                                clock.reset();
                            }
            ...
            {
                out_drain--;
                // PRX_WARN_S( (traj_cost<PRX_INFINITY) <<" "<< trajectory.in_collision()<<" "<<trajectory.size());
                if(!collision_checking || ( traj_cost<PRX_INFINITY && validity_checker->is_valid(trajectory) && trajectory.size()>1))
                {
                    double new_cost = get_vertex(nearest)->cost + traj_cost;
                    //check for better things
            ...
            iteration_count++;
            if(!real_solution && iteration_count%reset_count ==0)
            {
                PRX_INFO_S("Purging");
                foreach(tree_node_t* vertex, sample_graph.vertices())
                {
                    ((informed_sample_point_t*)vertex)->visiting_nodes.clear();
                    ((informed_sample_point_t*)vertex)->set = false;
                }
        ...
        void isst_t::purge_tree(util::tree_vertex_index_t v)
        {
            if(get_vertex(v)->get_children().size()!=0)
            {
                std::vector<tree_vertex_index_t> vs;
        ...
        void isst_t::prune_node2(util::tree_vertex_index_t v)
        {
            if(v!=start_vertex)
            {
        ...
        double isst_t::heuristic(const util::space_point_t* s)
        {
            ...
            {
                return validity_checker->heuristic(s,input_query->get_goal()->get_first_goal_point());
            }
        }
        ...
        {  
            ...
            {
                return validity_checker->heuristic(tree[s]->point,input_query->get_goal()->get_first_goal_point());
            }
        }
