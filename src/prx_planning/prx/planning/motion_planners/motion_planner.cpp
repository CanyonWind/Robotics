...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void motion_planner_t::link_specification(specification_t* new_spec)
        {
            ...
            //PRX_DEBUG_COLOR("Motion planner linking spaces of size: " << state_space->get_dimension() << " : " << control_space->get_dimension(), PRX_TEXT_LIGHTGRAY);
        ...
        void motion_planner_t::set_param(const std::string& parameter_name, const boost::any& value)
        {
            ...
            if( parameter_name == "serialize_flag" )
            {
                ...
                if(serialize_flag)
                    PRX_FATAL_S("This is not how we should do that. We should just specify the serialization file and the flag will become true");
            }
            else if( parameter_name == "deserialize_flag" )
            {
                deserialize_flag = boost::any_cast<bool>(value);
                if(deserialize_flag)
                    PRX_FATAL_S("This is not how we should do that. We should just specify the deserialization file and the flag will become true");
            }
