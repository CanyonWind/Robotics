...
PLUGINLIB_EXPORT_CLASS( prx::plan::rrt_star_t, prx::plan::planner_t)
...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void rrt_star_t::init(const parameter_reader_t* reader, const parameter_reader_t* template_reader)
        {
            rrt_t::init(reader, template_reader);
        }

        double rrt_star_t::compute_cost()
        {
            tree_vertex_index_t best_goal = nearest_vertex(input_query->get_goal()->get_first_goal_point());
            return get_vertex(best_goal)->cost;
        }
        void rrt_star_t::resolve_query()
        {
            if(input_query->q_collision_type == motion_planning_query_t::PRX_NO_COLLISIONS)
            {
                ...
                foreach(tree_vertex_index_t v, possible_goals)
                {
                    if(get_vertex(v)->cost < best_cost )
                    {
                        best_cost = get_vertex(v)->cost;
                        best_goal = v;
                    }
                }
                if(best_cost==PRX_INFINITY)
                    return;

                input_query->solution_cost = get_vertex(best_goal)->cost;

                local_planner->steer(get_vertex(best_goal)->point,input_query->get_goal()->get_first_goal_point(),input_query->plan,input_query->path,true);
...
//            sampler->sample(state_space, sample_point);
            ...
            // PRX_INFO_S("plan: "<<plan.print());
            // PRX_INFO_S("\n state: "<<state_space->print_point(sample_point)<<" \n traj: "<<state_space->print_point(trajectory.back()));
        ...
        void rrt_star_t::update_costs(tree_vertex_index_t v)
        {
        ...
        std::vector<tree_vertex_index_t> rrt_star_t::neighbors(state_t* state)
        {
            update_k();
        ...
        const statistics_t* rrt_star_t::get_statistics()
        {
            ...
            statistics->as<rrt_statistics_t > ()->solution_quality = input_query->solution_cost;
            ...
            PRX_INFO_S("Stats: "<<iteration_count<<" "<<tree.num_vertices()<<" "<<input_query->solution_cost );
            clock.reset();
            return statistics;
        }
