...
namespace prx
{
    ...
    namespace sim
    {
        ...
        void trajectory_t::save_to_file(std::string filename, unsigned precision)
        {
            PRX_ASSERT(state_space != NULL);
            ...
            {
                fout << state_space->print_point(state, precision) << "\n";
            }

            fout.close();
        }

        void trajectory_t::save_to_stream(std::ofstream& output_stream, unsigned precision)
        {
            PRX_ASSERT(state_space != NULL);
            PRX_DEBUG_COLOR("--Saving trajectory", PRX_TEXT_GREEN);
            ...
            foreach(state_t* state, *this)
            {
                output_stream << state_space->serialize_point(state, precision) << "\n";
            }
        ...
        {
            if( state_space != NULL && in_space != state_space )
            {
                for( unsigned i=0; i<states.size(); ++i )
                {
                    state_space->free_point( states[i] );
                }
                states.clear();
