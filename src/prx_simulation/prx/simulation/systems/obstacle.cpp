...
namespace prx
{
    ...
    namespace sim
    {
        ...
        void obstacle_t::link_collision_info(collision_checker_t* collision_checker)
        {
            int i =0;
            foreach(std::string config_name, geometries_names)
            {
        ...
        void obstacle_t::update_collision_info()
        {
            //no need to do this
        }
        ...
        {
            foreach(const config_list_element_t& element, configurations)
            {
        ...
        void obstacle_t::update_root_configuration(const config_t& new_root)
        {
            ...
            for(unsigned i=0;i<size;i++)
            {
                ...
                {
                    collision_infos[i].second = configurations[i].second;
                    collision_infos[i].first->update_matrices(collision_infos[i].second);
                }
            }
        }

        util::config_t obstacle_t::get_root_configuration()
        {
            return root_config;
        }
