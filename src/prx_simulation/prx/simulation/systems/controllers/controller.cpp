...
namespace prx
{
    ...
    namespace sim
    {
        ...
        void controller_t::link_collision_info(collision_checker_t* collision_checker)
        {
            for( hash_t<std::string, system_ptr_t >::const_iterator subsystems_const_iter = subsystems.begin(); subsystems_const_iter != subsystems.end(); ++subsystems_const_iter )
            {
                if( subsystems_const_iter->second->is_active() || simulation::update_all_configs )
                    subsystems_const_iter->second->link_collision_info(collision_checker);
            }
        }

        void controller_t::update_collision_info()
        {
            for( hash_t<std::string, system_ptr_t >::const_iterator subsystems_const_iter = subsystems.begin(); subsystems_const_iter != subsystems.end(); ++subsystems_const_iter )
            {
                if( subsystems_const_iter->second->is_active() || simulation::update_all_configs )
                    subsystems_const_iter->second->update_collision_info();
            }
        }
